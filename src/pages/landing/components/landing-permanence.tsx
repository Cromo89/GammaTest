...
function LevelCard({ level, delay }: LevelCardProps) {
  const isGamma = level.id === 'gamma'
  ...
  return (
    <div className="relative isolate flex flex-1">
      {isGamma && (
        <span
          aria-hidden="true"
          className="animate-card-glow-pulse pointer-events-none absolute -inset-1 -z-10 rounded-2xl bg-brand-teal/50 blur-xl"
        />
      )}
      <Reveal
        blur
        ...
        className={cn(
          'flex flex-1 flex-col rounded-2xl border p-6',
          isGamma ? 'border-brand-teal/70 bg-background' : 'border-border',
        )}
      >
...
export function LandingPermanence() {
  return (
    <section id="permanencia" className="mx-auto max-w-6xl scroll-mt-24 px-6 py-24 3xl:max-w-[1600px]">
      ...
      <div className="mt-16 flex flex-col items-stretch gap-4 sm:flex-row sm:items-end">
        {LEVELS.map((level, index) => (
          <LevelCard key={level.id} level={level} delay={150 + index * 120} />
        ))}
      </div>
    </section>
  )
