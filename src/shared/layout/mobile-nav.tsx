...
function MenuOverlay({ value, onChange, email, tier, onClose }: MenuOverlayProps) {
  return (
    <div className="fixed inset-0 z-50 flex flex-col overflow-y-auto bg-background lg:hidden">
      ...
      <div className="flex flex-1 flex-col gap-6 p-4">
        ...
        <div className="relative">
          <select
            className="h-10 w-full appearance-none rounded-lg border border-border bg-transparent py-2 pr-9 pl-3 text-sm text-foreground"
            defaultValue="es"
          >
...
export function MobileNav({ value, onChange, email, tier }: MobileNavProps) {
  ...
  return (
    <>
      ...
      {menuOpen && (
        <MenuOverlay value={value} onChange={onChange} email={email} tier={tier} onClose={() => setMenuOpen(false)} />
      )}
    </>
  )
}
